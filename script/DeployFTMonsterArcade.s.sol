...
contract DeployFTMonsterArcade is Script {
    ...
    error InvalidInitialPoolReference();
    ...
    function run() external {
        ...
        uint256 seedBuyEth = vm.envOr("ARCADE_SEED_BUY_ETH", uint256(0.001 ether));
        uint256 referenceEth = vm.envOr("ARCADE_INITIAL_POOL_ETH", seedBuyEth == 0 ? uint256(0.001 ether) : seedBuyEth);
        uint256 referenceTokenBudget = vm.envOr("ARCADE_INITIAL_POOL_TOKEN_BUDGET", uint256(0));
        ...
        if (seedBuyEth > 0) {
            seedTokenOut = launcher.buyGenesis{value: seedBuyEth}(deployer);
        }

        PoolKey memory poolKey = _poolKey(token, hook);
        if (vm.envOr("ARCADE_INITIALIZE_POOL", true)) {
            uint256 tokenBudget = referenceTokenBudget == 0 ? seedTokenOut : referenceTokenBudget;
            if (referenceEth == 0 || tokenBudget == 0) revert InvalidInitialPoolReference();
            uint160 initialSqrtPriceX96 = FTMonsterLaunchMath.initialSqrtPriceX96(referenceEth, tokenBudget);
            poolManager.initialize(poolKey, initialSqrtPriceX96);
        }

        vm.stopBroadcast();

        console2.log("ARCADE_DEPLOYER", deployer);
        console2.log("ARCADE_LAUNCHER", address(launcher));
        console2.log("ARCADE_TOKEN", address(token));
        console2.log("ARCADE_JACKPOT", address(jackpot));
        console2.log("ARCADE_PENALTY_DRAW", address(penaltyDraw));
        console2.log("ARCADE_LP_CLUB", address(lpClub));
        console2.log("ARCADE_HOOK", address(hook));
        console2.log("ARCADE_TAX_WALLET", taxWallet);
        console2.log("ARCADE_POOL_MANAGER", address(poolManager));
        console2.log("OFFICIAL_MAINNET_POSITION_MANAGER", DEFAULT_MAINNET_POSITION_MANAGER);
        console2.log("ARCADE_SEED_BUY_ETH_WEI", seedBuyEth);
    ...
    function _poolKey(FTMonsterArcadeToken token, FTMonsterArcadeV4Hook hook)
        internal
        pure
